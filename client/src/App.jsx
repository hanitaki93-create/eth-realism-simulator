      ...
      {result && (
        <>
          <div className="card" style={{ marginBottom:16 }}>
            ...
            <div><b>Entry mode:</b> {result.summary.entryMode}</div>
            <div><b>Execution model:</b> {result.summary.executionModel} (maker fill probability {fmt(result.summary.fillProb, 2)})</div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(12, minmax(0,1fr))', gap:10, marginBottom:16 }}>
            {[
              ...
              ['Maker', result.summary.makerFilled],
              ['Taker', result.summary.takerFilled],
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1.15fr 0.85fr', gap:16, marginBottom:16 }}>
            <div className="card">
              ...
              <table className="sim-table">
                <thead>
                  <tr>
                    <th>Engine</th><th>Raw</th><th>Blocked</th><th>Pending</th><th>Cancelled</th><th>Activated</th><th>Settled</th><th>Filled</th><th>Missed</th><th>Maker</th><th>Taker</th><th>Fallback</th><th>Wins</th><th>Losses</th><th>Net R</th>
                  </tr>
                </thead>
                <tbody>
                  {engineRows.map(([id, s]) => (
                    <tr key={id}>
                      ...
                      <td>{fmt(s.makerFilled,0)}</td>
                      <td>{fmt(s.takerFilled,0)}</td>
                      <td>{fmt(s.fallbackUsed,0)}</td>
                      <td>{fmt(s.wins,0)}</td>
            ...
            <div className="card">
              ...
              <div>Avg entry slip: {fmt(result.summary.avgEntrySlip, 2)} pts</div>
              <div>Avg SL slip: {fmt(result.summary.avgSLSlip, 2)} pts</div>
              <div>Avg TP slip: {fmt(result.summary.avgTPSlip, 2)} pts</div>
              <div>Maker entries: {fmt(result.summary.makerFilled, 0)}</div>
              <div>Taker entries: {fmt(result.summary.takerFilled, 0)}</div>
              <div>Taker fallbacks: {fmt(result.summary.fallbackUsed, 0)}</div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'0.9fr 1.1fr', gap:16 }}>
            ...
            <div className="card">
              ...
              <table className="sim-table">
                <thead>
                  <tr>
                    <th>Engine</th><th>Status</th><th>Side</th><th>Entry</th><th>Exit</th><th>Gross</th><th>PnL USD</th><th>PnL R</th><th>Entry fee</th><th>Exit fee</th><th>Entry type</th><th>Fallback</th><th>Entry slip</th><th>Entry notional</th><th>Exit notional</th>
                  </tr>
                </thead>
                <tbody>
                  {recentRows.map((r, idx) => (
                    <tr key={idx}>
                      ...
                      <td>{fmt(r.exitFeeUsd,2)}</td><td>{r.entryType}</td><td>{r.fallbackUsed ? 'yes' : 'no'}</td><td>{fmt(r.entrySlippagePts,2)}</td><td>{fmt(r.entryNotional,2)}</td><td>{fmt(r.exitNotional,2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
